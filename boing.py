...
class Bat(Actor):
    ...
    def update(self):
        self.timer -= 1
        y_movment = self.move_func()
        self.y = min(400, max(80, self.y +y_movment))
...
class Game:
    ...
    def update(self):
        ...
        if self.ball.out():
            ...
            loosing_player = 1 - scoring_player

            if self.bats[loosing_player].timer < 0:
                self.bats[scoring_player].score += 1
                game.play_sound("score_goal", 1)
                self.bats[loosing_player].timer = 20

            elif self.bats[loosing_player].timer == 0:
                direction = -1 if loosing_player == 0 else 1
